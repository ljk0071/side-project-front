...
export const useAuth = defineStore(
  ...
  () => {
    ...
    const logout = async () => {
      ...
      usePartyApplications().applications = []
      useResume().resetResume()

      usePopUpClosed().isPopUp = false
      useDiscordAuth().isClicked = false
      useNotFound().is404 = false
      useActiveParty().hasActiveParty = false
    }
